...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white border-b-8 border-white shadow-[0_25px_50px_-12px] shadow-amber-900/30 sticky top-0 z-50 font-vibes">
      <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-full max-w-7xl px-4">
        ...
        <div className="flex justify-between px-6">
          <GiForkKnifeSpoon className="text-black -mt-4 -ml-2 rotate-45" size={32} />
          <GiForkKnifeSpoon className="text-black -mt-4 -mr-2 -rotate-45" size={32} />
        </div>
      </div>
        ...
        <div className="flex justify-between items-center h-16 lg:h-20">
          ...
          <div className="flex-shrink-0 flex items-center space-x-2 group">
            ...
            <div className="flex flex-col ml-1 md:ml-2">
              <NavLink
                to="/"
                className="text-lg md:text-xl lg:text-2xl xl:text-3xl text-amber-700 bg-clip-text font-monsieur tracking-wider whitespace-nowrap"
              >
                SnackSwipe
              </NavLink>
          ...
          <div className="hidden lg:flex items-center space-x-2 xl:space-x-4 flex-1 justify-end">
            {navLinks.map((link) => (
              ...
              >
                <span className="text-amber-700">{link.icon}</span>
                <span className="text-amber-700">{link.name}</span>
              </NavLink>
            ))}
            <div className="flex items-center space-x-2 xl:space-x-4 ml-2 xl:ml-4">
              <NavLink
                to="/cart"
                className="p-2 relative text-amber-700 hover:text-amber-300 transition-colors"
              >
