...
import { api as apiUrl } from '../../config/api';
...
const OurMenuHome = () => {
  ...
  const displayItems = (() => {
    ...
    if (Array.isArray(active) && active.length) return [...active].sort((a, b) => Number(b.price) - Number(a.price)).slice(0, 4);
    const unc = menuData['Uncategorized'];
    if (Array.isArray(unc) && unc.length) return [...unc].sort((a, b) => Number(b.price) - Number(a.price)).slice(0, 4);
    const all = Object.values(menuData).flat?.() || [];
    return [...all].sort((a, b) => Number(b.price) - Number(a.price)).slice(0, 4);
  })();
