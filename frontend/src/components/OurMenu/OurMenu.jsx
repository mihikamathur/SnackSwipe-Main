...
import { api as apiUrl, API_BASE } from '../../config/api';
import { dummyMenuData } from '../../assets/OmDD';
import PlaceholderImg from '../../assets/AboutImage.png';
...
const OurMenu = () => {
  ...
  useEffect(() => {
    const fetchMenu = async () => {
      try {
        ...
        // if backend has no items yet, fall back to local dummy data (includes images)
        ...
        // on network error, also fall back to local dummy data
        setMenuData(dummyMenuData);
      }
    };
    fetchMenu();
  ...
  // items to display in active category (with optional search)
  const displayItems = useMemo(() => {
    const getAll = () => Object.values(menuData).flat?.() || [];
    ...
    if (Array.isArray(active) && active.length) return [...active].sort((a, b) => Number(b.price) - Number(a.price)).slice(0, 12);
    const unc = menuData['Uncategorized'];
    if (Array.isArray(unc) && unc.length) return [...unc].sort((a, b) => Number(b.price) - Number(a.price)).slice(0, 12);
    const all = getAll();
    return [...all].sort((a, b) => Number(b.price) - Number(a.price)).slice(0, 12);
            ...
            return (
              ...
              >
                ...
                <div className="relative h-48 sm:h-56 md:h-60 flex items-center justify-center bg-black/10">
                  <img
                    src={item.imageUrl || item.image || PlaceholderImg}
                    alt={item.name}
                    onError={(e) => {
